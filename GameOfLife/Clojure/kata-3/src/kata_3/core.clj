...
;;
;; На всякий случай, 
;; (f 1 "hello") - вызов функции f с параметрами 1 и "hello", 
...
;; Создадим неймспейс в котором будет 
;; находится все наши обьявленые функции.
;; Загрузим все из midje.sweet.
;; midje - супер няшная библеотека для тестирования 
;; https://github.com/marick/Midje 
;; midje.sweet - неймспейс в котором хранятся вещи нужные для тестов
(ns kata-3.core
  (:require [midje.sweet :refer :all]))

;; Обьявим что у нас есть empty-field. 
;; В Clojure это необходимо если мы хотим
;; где то использовать empty-filed до того как явно его опишем, 
;; см. http://clojuredocs.org/clojure_core/clojure.core/declare
;; На сколько я понимаю это все из за ограничени JVM, 
;; в ClojureScript можно и без declare
...
;; reduce принемает первым агрументом функцию, 
;; назовем ее f, f должна принемать 2 параметра 
;; и что то возвращать,  а вторым - колекцию, 
;; на первом шаге reduce применяет f к первым двум аргументам колекции,
;; полечает результат, 
;; на втором шаге reduce применяет к f предыдущему результу и третему элементу
;; начальной колекции, таким образом перебирается вся колекция, 
;; и в конце мы получаем результат вычеслений.
...
;;
;; У нас в качестве функции для reduce мы передаем анонимную ф-ю 
;; которая принемает доску и координату, анонимная функция
;; меняет нужный элемент на живой, это делается с помощью ф-ции ```assoc-in```. 
;; assoc-in принемает колекцию которая подвержит изминению, 
;; колекцию ключей (в нашем случае эти ключи как рза и будут
;; координатами), и элемент который будет установлен 
;; ```(assoc-in [[1 2] [3 4]] [1 0] 9)``` возьмет элемент с 
;; индексом 1 из изначальной колекции, это будет ```[3 4]```
...
;; Соседи это (Moore neighborhood), 
;; у каждой клетки на поле без ограничений всегда будет 8 соседей
...
;; Наша функция будет генерировать все возможные 
;; варианты изминения координат (кроме [0 0]) 
...
;;                                                   [1 1])
;;
;; теперь каждый вариант нужно применить к 
;; начальным координатам ```[(+ x dx) (+ y dy)]```
...
;; Теперь функция которая будет считать следующее поколение живых клеток, 
;; на основе существующих живых клеток

;; Как всегда...
(declare next-gen)

;; next-gen принемает множество живый клеток и 
;; возвращает множество живых клеток следующего поколения
